"""Schema-v4 encoded-video artifact audit contracts.
...
def _record(*, steps=2):
    ...
    latency = timing.LatencyEventV4(0, 25_000_000, 25_000_000, "success")
    activation = timing.PlanActivationV4(
        0, 0, 0, 25_000_000, "initial", {"action_cursor": 0}
    )
    stall = timing.ControlStallV4(
...
def _planned(record, *, include_stalls=True):
    return timing.build_video_timing_audit_v4(
...
def test_video_artifact_round_trip_crosschecks_the_exact_planned_timing_object():
    ...
    assert evaluation.VideoArtifactAuditV4.from_dict(audit.to_dict()) == audit
...
def test_duration_only_deviation_is_a_warning_but_fps_and_frame_mismatch_are_errors():
    record = _record()
    planned = _planned(record)
    warning = evaluation.build_video_artifact_audit_v4(
        episode=record,
        path="videos/example.mp4",
        planned=planned,
        video_show_inference_waits=True,
    ...
    for changes in (
        ...
        with pytest.raises(ValueError):
            evaluation.build_video_artifact_audit_v4(
                episode=record,
                path="videos/example.mp4",
                planned=planned,
                video_show_inference_waits=True,
                encoded_fps=changes.get("encoded_fps", 40),
                encoded_frame_count=changes.get(
                    "encoded_frame_count", planned.video_frame_count
                ),
                encoded_duration_s=planned.expected_duration_ns / 1_000_000_000,
            )
...
def test_only_an_empty_timeline_accepts_the_documented_one_frame_padding():
    ...
    audit = evaluation.VideoArtifactAuditV4(
        schema_version=4,
...
def test_video_audit_rejects_exact_field_bool_nonfinite_wrong_object_and_identity_mutations():
    ...
    for value in malformed:
        with pytest.raises(ValueError):
            evaluation.VideoArtifactAuditV4.from_dict(value)

    other_record = dataclasses.replace(record, execution_mode="baseline_sync_n5")
    with pytest.raises(ValueError, match="execution_mode"):
        audit.validate_episode(other_record)
...
def test_video_writer_serializes_the_nested_timing_audit_without_flattening(tmp_path):
    ...
    assert '"schema_version": 4' in persisted
