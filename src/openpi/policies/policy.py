...
class Policy(BasePolicy):
    def __init__(
        ...
        *,
        ...
        has_rtc_hook = self._sample_actions_rtc is not None
    ...
    def infer(self, obs: dict, *, noise: np.ndarray | None = None) -> dict:  # type: ignore[misc]
        ...
        is_pytorch_model = getattr(self, "_is_pytorch_model", False)
        ...
        if rtc_sample_call is None:
            ...
            if noise is not None:
                noise = torch.from_numpy(noise).to(self._pytorch_device) if is_pytorch_model else jnp.asarray(noise)
                if noise.ndim == 2:
                    noise = noise[None, ...]
