...
| 9 | `1b32660637610999ea6781f5bae27ea3ec3089e7` Fix LIBERO schema migration boundaries | implemented | `libero_eval_test.py`; `libero_report.py`; `libero_report_test.py` | Keep schema-v2 results archive-only, require v3-only identities for formal inputs, and reject version/field cross-contamination. |
...
| 22 | `b12650c62b3df8eec511a0fb1c14aede69a7a050` Harden schema-v3 identity and video audits | implemented | `libero_eval.py`; `libero_report.py`; retained report/video/host tests; slim runbook | Fail closed on schema-v3 evaluator Git/runtime identity, malformed timing events, mismatched stall-frame counts, missing stall-source frames, and incomplete overlay evidence. |
