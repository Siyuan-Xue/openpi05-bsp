...
def test_registry_contains_only_the_five_pi05_libero_configs():
    ...
    for name, config in configs.items():
        assert _called_name(_call_keyword(config, "data")) == "LeRobotLiberoDataConfig"
        model = _call_keyword(config, "model")
        if name.endswith("_lora_h16"):
            assert isinstance(model, ast.Name)
            assert model.id == "_PI05_LIBERO_LORA_H16_MODEL"
        else:
            assert _called_name(model) == "Pi0Config"
            assert ast.literal_eval(_call_keyword(model, "pi05"))


def test_h16_full_and_lora_recipes_preserve_short10k_invariants():
    configs = _train_configs()
    ...
    for suffix in ("baseline", "bsp"):
        ...
        for config in (full, lora):
            assert ast.literal_eval(_call_keyword(config, "seed")) == 42
            assert ast.literal_eval(_call_keyword(config, "batch_size")) == 256
            assert ast.literal_eval(_call_keyword(config, "micro_batch_size")) == 1
            assert ast.literal_eval(_call_keyword(config, "num_train_steps")) == 10000
            assert ast.literal_eval(_call_keyword(config, "save_interval")) == 1000
            assert ast.literal_eval(_call_keyword(config, "keep_period")) == 10000
            assert ast.literal_eval(_call_keyword(config, "permanent_checkpoint_steps")) == expected_milestones
        assert ast.literal_eval(_call_keyword(full, "ema_decay")) == 0.999
        assert ast.literal_eval(_call_keyword(lora, "ema_decay")) is None

    lora_model = _assignment(_tree(_CONFIG), "_PI05_LIBERO_LORA_H16_MODEL")
    assert _called_name(lora_model) == "Pi0Config"
    assert ast.literal_eval(_call_keyword(lora_model, "pi05"))
    assert ast.literal_eval(_call_keyword(lora_model, "action_dim")) == 32
    assert ast.literal_eval(_call_keyword(lora_model, "action_horizon")) == 16
    assert not (ast.literal_eval(_call_keyword(lora_model, "discrete_state_input")))
    assert ast.literal_eval(_call_keyword(lora_model, "paligemma_variant")) == "gemma_2b_lora"
    assert ast.literal_eval(_call_keyword(lora_model, "action_expert_variant")) == "gemma_300m_lora"
...
def test_tokenization_surface_is_paligemma_only():
    ...
    assert tokenizer_classes == {"PaligemmaTokenizer"}
    imports = {
...
def test_data_loader_supports_only_lerobot_and_bsp_paths():
    ...
    assert "create_torch_dataset" in names
    assert "BspLeRobotDataset" in (_ROOT / "src/openpi/training/data_loader.py").read_text()
    ...
    assert "framework" not in {arg.arg for arg in create_loader.args.args + create_loader.args.kwonlyargs}
